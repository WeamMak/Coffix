...
Build Task 19's authenticated Hebrew RTL home and shopping experience, while repairing the backend response gaps that currently prevent the approved Editorial home and accessible product imagery from being implemented against real API data.
...
The work does not include cart screens, checkout, catalog-media administration, favorites, ratings, reviews, promotions, or search. Those controls from the visual prototype must not be presented as working features.
...
`mobile/src/features/catalog/api.ts` is the network boundary. It exposes specific methods for activity summary, categories, paginated products, product detail, and adding a desired SKU quantity to the existing server-owned cart. The cart request body contains exactly `sku_id` and `quantity`; displayed price and stock are never sent as authority.
...
- Shop index and `categories` show the category grid.
...
The home screen follows the handoff's default Editorial hierarchy: brand/header, greeting, optional active-activity cards, featured product, category row, featured products, and service call-to-action. Sections with no data collapse cleanly. Failures provide a localized retry action.

Categories use a two-column photo-forward grid. Product lists use the approved editorial two-column cards. Product detail uses the leading product image, product/category identity, price, description, SKU attributes, availability, quantity stepper, and sticky add-to-cart action. Image elements always have meaningful Hebrew accessibility labels; decorative fallbacks are hidden from accessibility when adjacent text already names the resource.
...
1. Backend HTTP endpoints: customer authorization, cross-customer isolation, deterministic active-summary selection, hidden inactive catalog data, ordered media, safe image URLs, and OpenAPI drift.
2. Mobile route components: loading/error/empty content, activity and catalog rendering, category/product navigation, pagination, unavailable presentation, authentication expiry, and the exact cart payload.
