...
export function IntakeMachine({ manufacturer, model, onPress }: { manufacturer: string; model: string; onPress: () => void }) {
  const name = `${manufacturer} ${model}`;
  return <Pressable accessibilityRole="button" accessibilityLabel={`המכונה: ${name}`} onPress={onPress}>
    <Card style={styles.machine}>
      ...
      <View style={styles.grow}><Text align="end" variant="caption" color={colors.ink3}>עבור</Text><Text align="end" variant="sectionTitle">{name}</Text></View>
      <Feather name="chevron-left" size={18} color={colors.ink3} />
    </Card>
  </Pressable>;
}
    ...
    {types.map(type => {
      ...
      return <Pressable key={type.id} accessibilityRole="radio" accessibilityLabel={`${type.label_he}, ${formatIls(type.diagnostic_fee_agorot)}`} accessibilityState={{ checked: selected }} onPress={() => onSelect(type.id)}>
        <Card style={[styles.service, { borderColor: selected ? colors.ink : colors.line }]}>
          ...
          <View style={styles.grow}><Text align="end" variant="sectionTitle">{type.label_he}</Text><Text align="end" variant="caption" color={colors.ink3}>{type.tags_he.join(', ')}</Text></View>
...
export function IssueDescription({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  return <Card style={{ padding: spacing.lg, gap: spacing.sm }}>
    ...
    <View style={styles.row}><Text align="end" variant="caption" color={colors.ink3}>{value.length} / 4000</Text><Text align="end" variant="caption" color={colors.ink3}>10 תווים לפחות</Text></View>
  </Card>;
}
...
export function UrgencyChoices({ options, selectedId, onSelect }: { options: ServiceOptions['urgencies']; selectedId?: string; onSelect: (id: string) => void }) {
  return <View style={{ gap: spacing.sm }}>
    <Text align="end" variant="caption" color={colors.ink2}>דחיפות</Text>
    <View style={styles.urgencies}>{options.map(option => <Pressable key={option.id} accessibilityRole="radio" accessibilityLabel={`${option.name_he}, ${option.description_he}, +${option.surcharge_percent}%`} accessibilityState={{ checked: option.id === selectedId }} onPress={() => onSelect(option.id)} style={styles.urgency}>
      <Card style={{ flex: 1, padding: spacing.md, borderColor: option.id === selectedId ? colors.ink : colors.line }}>
        <View style={styles.row}><Text align="end" variant="sectionTitle">{option.name_he}</Text><Text align="end" variant="caption" color={colors.ink3}>{`+${option.surcharge_percent}%`}</Text></View>
        <Text align="end" variant="caption" color={colors.ink3}>{option.description_he}</Text>
      </Card>
    </Pressable>)}</View>
    <Text align="end" variant="caption" color={colors.ink3}>תוספת הדחיפות תחול על האבחון ועל עלות תיקון נוספת, אם תידרש.</Text>
  </View>;
}
...
export function IntakeSummary({ rows }: { rows: [string, string][] }) {
  return <Card style={{ paddingHorizontal: spacing.lg, paddingVertical: spacing.xs }}>
    {rows.map(([label, value], index) => <View key={label} style={[styles.summaryRow, index < rows.length - 1 && styles.divider]}>
      <Text align="end" variant="caption" color={colors.ink3}>{label}</Text><Text style={{ flex: 1 }} align="start">{value}</Text>
    </View>)}
  </Card>;
}
...
export function DiagnosticReviewNotice() {
  return <View style={styles.notice}>
    ...
    <Text align="end" variant="caption" color={colors.accentDeep} style={styles.grow}>תזכורת: אגרת האבחון תיקבע לאחר סקירת הצוות. נשלח אליכם הצעה לתשלום לפני תחילת העבודה.</Text>
  </View>;
}
