...
const rows: { label: string; icon: ComponentProps<typeof Feather>['name']; href: Href }[] = [
  { label: 'פרטים אישיים', icon: 'user', href: '/(tabs)/(profile)/personal' },
...
const generalRows: typeof rows = [
  { label: 'שאלות נפוצות', icon: 'info', href: '/(tabs)/(profile)/faq' },
  { label: 'צרו קשר', icon: 'phone', href: '/(tabs)/(profile)/contact' },
  { label: 'הגדרות', icon: 'settings', href: '/(tabs)/(profile)/settings' },
];
export function ProfileContent({ sessionScope, logout }: { sessionScope: string; logout(): Promise<void> }) {
  ...
  return <Screen scroll contentContainerStyle={styles.screen} header={<View style={styles.header}>
    <Text accessibilityRole="header" variant="screenTitle" style={{ flex: 1 }}>הפרופיל שלי</Text>
  </View>}>
    ...
    <View style={styles.group}>{rows.slice(0, 2).map(row => <ProfileRow key={row.label} {...row} />)}</View>
    <Text color={colors.ink2} variant="eyebrow">הפעילות שלי</Text>
    <View style={styles.group}>{rows.slice(2).map(row => <ProfileRow key={row.label} {...row} />)}</View>
    <Text color={colors.ink2} variant="eyebrow">כללי</Text>
    <View style={styles.group}>{generalRows.map((row, index) => <ProfileRow key={row.label} {...row} divider={index > 0} />)}</View>
...
function ProfileRow({ label, icon, href, divider = false }: typeof rows[number] & { divider?: boolean }) {
  return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={() => router.push(href)} style={[styles.row, divider ? { borderTopWidth: 1, borderTopColor: colors.line } : null]}>
    <View style={[styles.rowIcon, icon === 'bell' ? { borderRadius: 16 } : null]}><Feather name={icon} size={18} color={colors.ink} /></View><Text style={{ flex: 1 }}>{label}</Text><Feather name="chevron-left" size={18} color={colors.ink2} />
  </Pressable>;
}
