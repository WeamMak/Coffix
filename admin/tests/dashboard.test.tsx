...
it('renders authoritative totals independently of the bounded recent-order list', async () => {
  const fetcher = commercePage('/overview', (url) => Response.json(url.pathname.endsWith('/orders') ? [] : { product_revenue_agorot: 123456, open_services: 17, awaiting_payment_orders: 3, awaiting_payment_services: 4, users_by_role: { technician: 2 }, orders_by_state: { paid: 5 }, service_requests_by_state: { diagnosing: 7 }, failed_deliveries: 2, failed_outbox_events: 6, pending_outbox_events: 9, low_stock_skus: 1, todays_appointments: [{ id: 'service-1', reference: 'SVC-TODAY', technician_name: 'Dana', start: '2026-09-08T07:00:00Z', end: '2026-09-08T08:00:00Z' }] }));
  expect(await screen.findByText(/1,234.56/)).toBeVisible();
  expect(within(screen.getByLabelText('בקשות שירות פתוחות')).getByText('17')).toBeVisible();
  expect(within(screen.getByLabelText('אירועי רקע שנכשלו')).getByText('6')).toBeVisible();
  expect(screen.getByRole('link', { name: 'SVC-TODAY' })).toHaveAttribute('href', '/service/service-1');
  expect(within(screen.getByLabelText('הזמנות הממתינות לטיפול')).getByText('5')).toBeVisible();
  expect(fetcher.mock.calls.some(([url]) => String(url).endsWith('/orders?limit=4&page=1'))).toBe(true);
  expect(fetcher.mock.calls.every(([url]) => /\/(dashboard|refresh)$|\/orders\?limit=4&page=1$/.test(String(url)))).toBe(true);
});
