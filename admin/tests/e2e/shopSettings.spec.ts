...
// The shared fixture creates a disposable database; shutdown drops only that
// database and its temporary media, including after failed assertions.
test.afterEach(async ({ request }) => {
  await request.post('http://127.0.0.1:8299/api/v1/__image_test_shutdown').catch(() => undefined);
  await expect.poll(async () => {
    try { return (await request.get('http://127.0.0.1:8299/api/v1/__image_test_sessions')).ok(); }
    catch { return false; }
  }).toBe(false);
});

test('edits shop settings and verifies customer contact, checkout, intake and role boundaries', async ({ page, request }) => {
  ...
  await page.screenshot({ path: 'test-results/shop-settings-review-phone.png', fullPage: true });
  ...
  await page.screenshot({ path: 'test-results/shop-settings-desktop.png', fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true);
  await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
  await page.screenshot({ path: 'test-results/shop-settings-phone.png', fullPage: true });
