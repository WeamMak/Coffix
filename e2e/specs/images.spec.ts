...
test('manages model/category/product images and enforces customer/technician permissions', async ({ page, request }, info) => {
  ...
  await page.screenshot({ path: info.outputPath('images-model.png'), fullPage: true });
  ...
  await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
  await page.screenshot({ path: info.outputPath('images-gallery-desktop.png'), fullPage: true });
  await page.setViewportSize({ width: 390, height: 844 });
  await page.evaluate(() => window.scrollTo({ top: 0, behavior: 'instant' }));
  await page.screenshot({ path: info.outputPath('images-gallery-phone.png'), fullPage: true });
