...
export async function paymentEvent(request: APIRequestContext, objectId: string, eventId: string, refund = false) {
  const body = JSON.stringify({ id: eventId, type: refund ? 'refund.updated' : 'payment_intent.succeeded', data: { object: { id: objectId, status: 'confirmed' } } });
